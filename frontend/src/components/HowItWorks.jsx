...
const HowItWorks = () => {
  ...
  }, [activeTab, cmsGuestSteps, cmsHostSteps, sectionImagesGuests, sectionImagesHosts, heroSlides, paused, reduceMotion, how?.mediaIntervalMs]);

  return (
    <div className="bg-white py-16 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2">{how.title}</h2>
          <p className="text-gray-600 text-lg">{how.subtitle}</p>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="flex items-center justify-center mb-2">
            <div className="inline-flex items-center bg-gray-100 rounded-full p-1">
              <button onClick={() => setActiveTab('guests')} className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${activeTab==='guests' ? 'bg-white text-gray-900 shadow' : 'text-gray-700 hover:text-gray-900'}`}>For Guests</button>
              <button onClick={() => setActiveTab('hosts')} className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${activeTab==='hosts' ? 'bg-white text-gray-900 shadow' : 'text-gray-700 hover:text-gray-900'}`}>For Hosts</button>
            </div>
          </div>
          ...
          <div
            className="relative rounded-2xl overflow-hidden bg-white/60 backdrop-blur shadow-md mb-8"
          ...
          >
            {(() => {
              ...
              if (count === 0) {
                return <div className="aspect-video bg-gray-100 flex items-center justify-center text-gray-400">Section Media</div>;
              }
              return (
                <div className="aspect-video">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3 z-10">
                    <div className="bg-white/85 backdrop-blur rounded-xl p-3 shadow">
                      <div className="flex items-center gap-2 mb-1">
                        {(() => { const Ico = slides[slideIndex]?.Icon; return Ico ? <Ico className="text-blue-600 text-sm" /> : null; })()}
                        <div className="text-sm font-semibold text-gray-800 truncate">{slides[slideIndex]?.title}</div>
                      </div>
                      <div className="text-xs text-gray-600 line-clamp-2">{slides[slideIndex]?.description}</div>
                    </div>
                  </div>
                  {count > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 text-gray-800 shadow hover:bg-white z-10 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        ‹
                      </button>
                      <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 text-gray-800 shadow hover:bg-white z-10 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        ›
                      </button>
                      <div className="absolute bottom-3 left-0 right-0 flex items-center justify-center gap-2 z-10">
                        {slides.map((_, i) => (
                          <button key={i} onClick={() => setSlideIndex(i)} aria-label={`Go to media ${i+1}`} className={`w-2.5 h-2.5 rounded-full ${i===slideIndex ? 'bg-blue-600' : 'bg-white/70'} focus:outline-none focus:ring-2 focus:ring-blue-500`} />
                        ))}
                      </div>
                    </>
                  )}
        ...
        {/* CTA for hosts */}
        <div className="rounded-2xl p-8 bg-blue-50/80 backdrop-blur shadow-md text-center">
          ...
          <p className="text-gray-600 text-sm mt-2">Sign up to start listing your apartment</p>
        </div>

        {/* Stats Section */}
        <div ref={statsRef} className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className={`relative p-6 rounded-2xl text-center bg-white/90 backdrop-blur shadow-xl hover:shadow-2xl transition-all duration-500 ${statsInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
               style={{ transitionDelay: '0ms' }}>
            <div className="absolute top-3 left-3 w-2 h-2 bg-blue-200 rounded"></div>
            <div className="mx-auto mb-2 w-10 h-10 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center"><FaBuilding /></div>
            <div className="text-4xl font-extrabold text-blue-700 tracking-tight">{(animated.activeListings ?? 0).toLocaleString?.() || animated.activeListings || 0}</div>
            <div className="text-xs mt-1 text-gray-500 uppercase tracking-wide">Active Listings</div>
          </div>
          <div className={`relative p-6 rounded-2xl text-center bg-white/90 backdrop-blur shadow-xl hover:shadow-2xl transition-all duration-500 ${statsInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
               style={{ transitionDelay: '120ms' }}>
            <div className="absolute top-3 left-3 w-2 h-2 bg-blue-200 rounded"></div>
            <div className="mx-auto mb-2 w-10 h-10 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center"><FaSmile /></div>
            <div className="text-4xl font-extrabold text-blue-700 tracking-tight">{(animated.happyGuests ?? 0).toLocaleString?.() || animated.happyGuests || 0}</div>
            <div className="text-xs mt-1 text-gray-500 uppercase tracking-wide">Happy Guests</div>
          </div>
          <div className={`relative p-6 rounded-2xl text-center bg-white/90 backdrop-blur shadow-xl hover:shadow-2xl transition-all duration-500 ${statsInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
               style={{ transitionDelay: '240ms' }}>
            <div className="absolute top-3 left-3 w-2 h-2 bg-blue-200 rounded"></div>
            <div className="mx-auto mb-2 w-10 h-10 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center"><FaThumbsUp /></div>
            <div className="text-4xl font-extrabold text-blue-700 tracking-tight">{animated.satisfactionRate ?? 0}%</div>
            <div className="text-xs mt-1 text-gray-500 uppercase tracking-wide">Satisfaction Rate</div>
          </div>
        </div>
