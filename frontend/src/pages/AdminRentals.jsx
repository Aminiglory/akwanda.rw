...
export default function AdminRentals() {
  ...
  async function loadContent() {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/api/admin/rentals-content`, { credentials: 'include' });
      if (!res.ok) throw new Error('Failed to fetch rentals content');
  ...
  async function saveContent() {
    try {
      setSaving(true);
      const res = await fetch(`${API_URL}/api/admin/rentals-content`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(content),
      });
      if (!res.ok) throw new Error('Failed to save rentals content');
      const data = await res.json();
      setContent(data.content || content);
      toast.success('Rentals content saved');
    } catch (e) {
      toast.error(e.message);
    } finally {
      setSaving(false);
  ...
  async function uploadImages(files) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/rentals-content/images`, {
        method: 'POST',
        credentials: 'include',
        body: form,
      });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-4">Admin: Rentals Page</h1>
      {loading ? (
