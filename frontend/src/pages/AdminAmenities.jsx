...
export default function AdminAmenities() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setForm({ name: '', slug: '', scope: 'property', type: 'amenity', icon: '', description: '', active: true, order: 0 });
      setEditingId(null);
      load();
  ...
  const startEdit = (it) => {
    setEditingId(it._id);
    setForm({ name: it.name, slug: it.slug, scope: it.scope, type: it.type, icon: it.icon || '', description: it.description || '', active: !!it.active, order: it.order || 0 });
  };
    ...
    <div className="max-w-5xl mx-auto p-6">
      ...
      <form onSubmit={submit} className="bg-white rounded-xl shadow p-4 mb-6 grid grid-cols-1 md:grid-cols-6 gap-3">
        ...
        <div className="md:col-span-6 flex gap-2">
          ...
          {editingId && <button type="button" onClick={()=>{ setEditingId(null); setForm({ name: '', slug: '', scope: 'property', type: 'amenity', icon: '', description: '', active: true, order: 0 }); }} className="px-4 py-2 border rounded flex items-center gap-2"><FaTimes /> Cancel</button>}
        </div>
      </form>
          ...
          {items.map(it => (
            <div key={it._id} className="p-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                {it.description && <div className="text-xs text-gray-500 max-w-xs truncate" title={it.description}>{it.description}</div>}
              </div>
